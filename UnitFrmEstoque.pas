...
var
  ...
  Valorbase,LocDestino,P_estoqueext,QtdP:real;
  ...
  IMPRESSORA:TextFile;
  qrytot,qryN,qrymun,qry : TZQuery;

implementation

uses Unitdm, UnitFrmCadClientes, UnitFrmCadfornecedores,
  UnitFrmPrincipal, UnitFrmEstoqueRel, UnitFrmCadProdutos, UEnvioNfe;
...
procedure TFrmEstoque.BitBtn5Click(Sender: TObject);
begin
  try
    ...
    fEnvioNfe.lbtipo.Caption:=copy(cbtipo.Text,1,2);
    ...
    qryN.SQL.Add('select documento from nf');
    qryN.sql.add('where tipo ='+chr(39)+'S'+chr(39));
    qryN.sql.add('order by documento desc');
    qryN.Open;

    if qryN.RecordCount>0 then
       fEnvioNfe.edNota.Text:=IntToStr(qryN.FieldByName('documento').AsInteger+1)
    else
    ...
    qrytot.SQL.Add('where documento ='+chr(39)+eddocumento.Text+chr(39));
    qrytot.Open;
    ...
    fEnvioNfe.lbvenda.Caption:=eddocumento.Text;
    fEnvioNfe.showmodal;
  finally
    fEnvioNfe.Release;
    fEnvioNfe:=nil;
