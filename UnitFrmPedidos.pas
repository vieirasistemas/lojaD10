...
//         begin
//           MensagemLiberacao:='Cliente com Carência de Atraso';
//           AtualizarVencidos;
        ...
        if dm.produtos.RecordCount>0 then
           begin
             ...
             if qrmovpedido.Locate('produto',edcodigo.Text,[]) then
                begin
                  if application.MessageBox('Lançar Novamente','Produto já Lançado',mb_yesno+mb_iconquestion+MB_DEFBUTTON2)=id_yes then
                     begin
...
begin
{
  ...
  begin
     ...
     begin
        application.MessageBox('Unidade Inválida','Atenção',mb_ok+MB_ICONERROR);
...
procedure Liberacao;
begin
      if application.messagebox('Confirma Liberação do Pedido?','Atenção',mb_yesno+mb_iconquestion)=id_yes then
         begin
           dm.parametros.Close;
           dm.parametros.Open;
           check:=true;
...
//                   dm.movestoqueicms.Value:=dm.movpedidoicms.Value;
           ...
           if dm.pedidostipo.Value='SS' then
              begin
                busca := '';
                ClickedOK := InputQuery('Nota de Serviço', 'Digite o Nº da Nota de Serviço', busca);
                if ClickedOK then
                   begin
                     FrmPedidos.qratualizar.Close;
                     FrmPedidos.qratualizar.SQL.Clear;
...
begin
      if not qrpedidoliberacao.IsNull then
         application.messagebox('Pedido já Liberado','Operação Impossível',mb_ok+MB_ICONINFORMATION)
      else
      begin
        dm.pedidos.Close;
        dm.pedidos.SQL.Clear;
        ...
        if dm.pedidosformapagto.IsNull then
           begin
             application.MessageBox('Efetue o Fechamento do Pedido','Atenção',mb_Ok+MB_ICONWARNING);
             ...
             if liberar=false then
                begin
                  ...
                  if dm.usuariospedidos.value='5' then
                     begin
                       if application.messagebox('Confirma Liberação?',StrToPChar(MensagemLiberacao),mb_yesno+MB_ICONQUESTION)=id_yes then
                          Liberacao;
                     end
...
begin
   if application.MessageBox('Confirma Exclusão?','Excluir Pedido',mb_yesno+mb_iconquestion+MB_DEFBUTTON2)=id_yes then
      begin
        qratualizar.Close;
        qratualizar.SQL.Clear;
        qratualizar.SQL.Add('delete from movpedido');
...
begin
  ...
  if not dm.pedidosliberacao.IsNull then
     begin
       showmessage('Pedido já enviado pro Caixa!!!');
       exit;
     end;
        ...
        if cbunidade.Text<>dm.produtosundint.Value then
           begin
              ...
              if vlrunit<vlrproduto then
                 begin
                    if application.messagebox('Deseja Proseguir','Preço inferior a tabela de venda',mb_yesno+mb_iconquestion)=id_no then
                       edunitario.SetFocus
                 end;
           end
           else
...
begin
      ...
      if key=vk_f7 then
         begin
           busca := '';
           ClickedOK := InputQuery('Digite a Observação', 'Observação do Pedido', busca);
           if ClickedOK then
              begin
                if busca<>'' then
                   begin
...
begin
  edqtd.Color:=$0080FFFF;
      if edcodigo.Text='' then
         begin
           application.MessageBox('Escolha um Produto','Atenção',mb_ok+MB_ICONERROR);
           edcodigo.SetFocus;
         end;
end;
...
begin
  if qrpedidoliberacao.IsNull then
     begin
       application.MessageBox('Libere o Pedido','Atenção',mb_ok+MB_ICONINFORMATION);
       btliberar.SetFocus;
     end
     else
     begin
...
begin
  if cbvendedor.Text<>'' then
     ...
     begin
       application.MessageBox('Escolha um Vendedor(a)','Atenção',mb_ok+MB_ICONERROR);
       cbvendedor.SetFocus;
     end;
...
begin
{  if application.MessageBox('Confirma Operação?','Gerar N.F.C.e',mb_yesno+mb_iconquestion+MB_DEFBUTTON2)=id_yes then
     begin
       dm.venda.Close;
       dm.venda.SQL.Clear;
...
begin
  ...
  if not dm.pedidosliberacao.IsNull then
     begin
       if application.messagebox('Deseja Liberar pra Enviar Novamente?','Pedido já enviado pro Caixa!!!',mb_yesno+mb_iconquestion)=id_yes then
          begin
            P_Liberado:='N';
            FrmPedidos.qratualizar.Close;
            FrmPedidos.qratualizar.SQL.Clear;
