...
begin
  ...
  FrmCadClientes.edfantasia.Text:=dm.clientesfantasia.value;
  FrmCadClientes.EdEndereco.Text:=dm.Clientesendereco.value;
  FrmCadClientes.EdBairro.Text:=dm.Clientesbairro.value;
  FrmCadClientes.cbcidade.Text:=dm.Clientescidade.value;
...
begin
  ...
  FrmCadClientes.edfantasia.Text:='';
  FrmCadClientes.EdEndereco.Text:='';
  FrmCadClientes.EdBairro.Text:='';
  FrmCadClientes.cbcidade.Text:='';
...
begin
  ...
  qry.SQL.Add('select UF from municipios');
  qry.SQL.Add('group by UF');
  qry.Open;
  cbestado.Items.Clear;
  while not qry.Eof do
        begin
          cbestado.Items.Add(qry.fieldbyname('UF').AsString);
          qry.Next;
        end;
  qry.Free;
...
begin
  if novo=true then
     begin
       ...
       qratualizar.SQL.Add('CALL sp_i_clientes(:c_cpf, :c_rg, :c_nome, :c_endereco, :c_bairro, :c_cidade, :c_estado, :c_cep, :c_fone1, :c_fone2, :c_fone3, :c_sexo, :c_dtnasc, :c_email, :c_instagran, :c_fantasia )');
       ...
       qratualizar.Params[5].Value := cbcidade.Text;
       ...
       if medtnasc.Text<>'  /  /    ' then
          ...
          qratualizar.Params[12].Value := StrToDate('00/00/0000');
       qratualizar.Params[13].Value := Edemail.Text;
       qratualizar.Params[14].Value := edhome.Text;
       qratualizar.Params[15].Value := edfantasia.Text;
       qratualizar.ExecSQL;
     ...
     begin
       qratualizar.Close;
       qratualizar.SQL.Clear;
       qratualizar.SQL.Add('CALL sp_a_clientes(:c_codigo, :c_cpf, :c_rg, :c_nome, :c_endereco, :c_bairro, :c_cidade, :c_estado, :c_cep, :c_fone1, :c_fone2, :c_fone3, :c_sexo, :c_dtnasc, :c_email, :c_instagran, :c_fantasia)');
       qratualizar.Params[0].Value := EdCodigo.Text;
       qratualizar.Params[1].Value := edcpf.Text;
       qratualizar.Params[2].Value := edrg.Text;
       qratualizar.Params[3].Value := ednome.Text;
       qratualizar.Params[4].Value := EdEndereco.Text;
       qratualizar.Params[5].Value := EdBairro.Text;
       qratualizar.Params[6].Value := cbcidade.Text;
       qratualizar.Params[7].Value := cbestado.Text;
       qratualizar.Params[8].Value := mecep.Text;
       qratualizar.Params[9].Value := mefone1.Text;
       qratualizar.Params[10].Value := mefone2.Text;
       qratualizar.Params[11].Value := mefone3.Text;
       if cbsexo.Text<>'' then
          qratualizar.Params[12].Value := copy(cbsexo.Text,1,1)
       else
          qratualizar.Params[12].Value := '';
       if medtnasc.Text<>'  /  /    ' then
          qratualizar.Params[13].Value := StrToDate(medtnasc.Text)
       else
          qratualizar.Params[13].Value := StrToDate('00/00/0000');
       qratualizar.Params[14].Value := Edemail.Text;
       qratualizar.Params[15].Value := edhome.Text;
       qratualizar.Params[16].Value := edfantasia.Text;
       qratualizar.ExecSQL;
      end;
  habilita;
  R_hora:=TimeToStr(time);
     ...
     begin
        if assigned(FrmReceber) then
           ...
           begin
             if (dm.Clientes.Active) and (dm.Clientes.RecordCount>0) then
                begin
                  FrmPedidos.edcliente.Text:=IntToStr(dm.clientescodigo.value);
                  FrmPedidos.edcliente.SelStart:=0;
                  FrmPedidos.edcliente.SelLength:=length(FrmPedidos.edcliente.Text);
                  modalresult := mrok;
                end
           end
...
begin
  ...
  if cbestado.Text<>'' then
     begin
        qry              :=  TZQuery.Create(nil);
        qry.Connection   :=  dm.ZConnection1;
        qry.SQL.Add('select Municipio from municipios');
        qry.sql.add('where UF = '+chr(39)+cbestado.Text+chr(39));
        qry.Open;
        while not qry.Eof do
              begin
                cbcidade.Items.Add(qry.fieldbyname('Municipio').AsString);
                qry.Next;
              end;
        qry.Free;
     end;
...
procedure TFrmCadClientes.cbcidadeEnter(Sender: TObject);
begin
  cbcidade.Color:=$0080FFFF;
end;

procedure TFrmCadClientes.cbcidadeExit(Sender: TObject);
begin
  cbcidade.Color:=clWindow;
end;

procedure TFrmCadClientes.edfantasiaEnter(Sender: TObject);
begin
  edfantasia.Color:=$0080FFFF;
end;

procedure TFrmCadClientes.edfantasiaExit(Sender: TObject);
begin
