...
type
  TFrmCadProdutos = class(TForm)
    ...
    AlteraroNCMdosprodutos1: TMenuItem;
    N2: TMenuItem;
    ...
    procedure AlteraroNCMdosprodutos1Click(Sender: TObject);
    procedure edncmEnter(Sender: TObject);
...
uses Unitdm, UnitFrmPrincipal, UnitFrmCadFornecedores, UnitFrmEstoque,
  ...
  UnitFrmGrupo, UnitFrmVendaRefaturar, UnitFrmPedidos;
    ...
    if novo=true then
       begin
         ...
         if edcodbarra.Text<>'' then
            dm.produtoscodbarra.value:=edcodbarra.Text;
         dm.produtosreferencia.value:=Edreferencia.Text;
         dm.produtosncm.value:=edncm.Text;
         if edqtd.Text<>'' then
       ...
       begin
         ...
         ':p_undext, :p_estmin, :p_estmax, :p_prazores, :p_fornecedor, :p_ativo, :p_compra, :p_venda, :p_atacado, :p_comissao, :p_ncm)');
         ...
         qratualizar.Params[17].Value := edncm.Text;
         qratualizar.ExecSQL;
       end;
      R_hora:=TimeToStr(time);
      qratualizar.Close;
     ...
     begin
        if assigned(FrmEstoque) then
           ...
           begin
             if (dm.produtos.Active) and (dm.produtos.RecordCount>0) then
                begin
                  FrmVendaRefaturar.edcodbarra.Text:=IntToStr(dm.produtoscodigo.value);
                  FrmVendaRefaturar.edcodbarra.SelStart:=0;
                  FrmVendaRefaturar.edcodbarra.SelLength:=length(FrmVendaRefaturar.edcodbarra.Text);
                  modalresult := mrok;
                end;
           end
           ...
           begin
             if (dm.produtos.Active) and (dm.produtos.RecordCount>0) then
                begin
                  FrmPedidos.edcodigo.Text:=IntToStr(dm.produtoscodigo.value);
                  FrmPedidos.edcodigo.SelStart:=0;
                  FrmPedidos.edcodigo.SelLength:=length(FrmPedidos.edcodigo.Text);
                  modalresult := mrok;
                end;
           end
...
procedure TFrmCadProdutos.AlteraroNCMdosprodutos1Click(Sender: TObject);
begin
  if dm.produtos.RecordCount<1 then
    begin
      showmessage('Consulte um Produto!!!');
  ...
  busca := '';
  ClickedOK := InputQuery('Digite o NCM do(s) Produto(s)', 'Alterar o NCM do(s) Produto(s)', busca);
  if ClickedOK then
    begin
      while not dm.produtos.Eof do
            begin
              ...
              qratualizar.Params[3].Value := 'PRODUTOS';
              qratualizar.Params[4].Value := 'A';
              qratualizar.Params[5].Value := 'NCM: '+busca;
              qratualizar.Params[6].Value := dm.produtoscodigo.AsInteger;
              qratualizar.Params[7].Value := 0;
              qratualizar.Params[8].Value := '';
              qratualizar.ExecSQL;
...
procedure TFrmCadProdutos.edncmEnter(Sender: TObject);
begin
  edncm.Color:=$0080FFFF;
end;

procedure TFrmCadProdutos.edncmExit(Sender: TObject);
begin
  edncm.Color:=clWindow;
end;

procedure TFrmCadProdutos.edcodbarraEnter(Sender: TObject);
begin
  edcodbarra.Color:=$0080FFFF;
end;

procedure TFrmCadProdutos.edcodbarraExit(Sender: TObject);
begin
