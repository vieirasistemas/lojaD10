...
begin
  ...
  if key=vk_f2 then
     begin
       busca := '';
       ClickedOK := InputQuery('Digite a Nova Taxa', 'Alterar Taxa do Cartão', busca);
       if ClickedOK then
          begin
            R_hora:=TimeToStr(time);
            dm.rastro.Close;
  ...
  if key=vk_f3 then
     begin
       ...
       ClickedOK := InputQuery('Digite o Novo Nome', 'Alterar Nome do Cartão', busca);
       if ClickedOK then
          begin
            R_hora:=TimeToStr(time);
            dm.rastro.Close;
