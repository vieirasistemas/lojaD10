...
begin
  try
    ...
    FrmPrincipal.Caption := 'Gestor Loja - Sistema Integrado de Gestão - Empresa: '+dm.parametrosfantasia.Value;
{
    try
...
begin
  if Is4DigitYear=false then
     ...
     begin
       ...
       if dias=0 then
          begin
            application.messagebox('Contate a Vieira Sistemas','Atenção! Sistema Expirado',MB_OK+MB_ICONINFORMATION);
            try
          ...
          application.messagebox('Contate o Suporte Técnico para Revalidar a Chave do Sistema','Atenção! Sistema Expira em Poucos Dias',MB_OK+MB_ICONINFORMATION);
     end;
end;
